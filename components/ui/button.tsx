...
export default function Button({
    ...
    const variantClass = {
        default: "bg-black text-white hover:bg-gray-800 disabled:text-gray-300",
        outline: "border bg-white hover:bg-gray-100 disabled:text-gray-700",
        ghost: "hover:bg-gray-100 bg-white disabled:text-gray-700",
        danger: "bg-red-500 text-white hover:bg-red-600 disabled:text-gray-300",
    ...
    >
        ...
        onClick={(e) => {
            ...
            if (animation) wave(e, accentuate, expandDuration, collapseDuration, frequency)
        }}
        {...props}
    >
        {children}
...
const wave = (e: React.MouseEvent<HTMLButtonElement>, accentuate: boolean, expandDuration: number, collapseDuration: number, frequency: number) => {
    ...
    // Get tailwind classes from button
    const buttonClasses = Array.from(buttonEl.classList).filter(className => className.includes("bg-"))

    // Get tailwind colors form tailwind classes
    const buttonColors = {
        bgColor: buttonClasses
            .filter(className => !className.includes("hover"))
            .toString()
            .replace("bg-", "")
            .split("-"),
        hovColor: buttonClasses
            .filter(className => className.includes("hover"))
            .toString()
            .replace("hover:bg-", "")
            .split("-")
    ...
    // Get tailwind color names and numbers
    const name = [buttonColors.bgColor[0], buttonColors.hovColor[0]]
    const number = [buttonColors.bgColor[1], accentuate ? 500 : buttonColors.hovColor[1]]

    // Get hexadecimal colors code form tailwind color names and numbers
    const { bgColor, hovColor } = {
        bgColor: number[0] ?
            twColorToHexa[name[0]][number[0]].toString() :
            twColorToHexa[name[0]].toString(),
        hovColor: number[1] ?
            twColorToHexa[name[1]][number[1]].toString() :
            twColorToHexa[name[1]].toString()
    }
